...
# Script to get current logs from the pipecat-daily-bot-stg workspace service container
# Usage: ./scripts/get-pipecat-daily-bot-stg-logs.sh [options]
...
Get current logs from the pipecat-daily-bot-stg workspace service container.
...
OPTIONS:
    ...
    -n, --namespace NAME   Kubernetes namespace (default: pipecat-daily-bot-{env})
...
if [ "$CLOUDWATCH" = true ]; then
    ...
    # Determine container name for CloudWatch filter
    CW_CONTAINER=""
    if [ -n "$CONTAINER_NAME" ]; then
        CW_CONTAINER="$CONTAINER_NAME"
    elif [ -n "$SERVICE_NAME" ]; then
        # Heuristic mapping for known services
        if [ "$SERVICE_NAME" == "operator" ]; then
            CW_CONTAINER="operator"
        elif [ "$SERVICE_NAME" == "gateway" ]; then
            CW_CONTAINER="bot"
        elif [ "$SERVICE_NAME" == "warm-pool" ]; then
            CW_CONTAINER="runner"
        else
            CW_CONTAINER="$SERVICE_NAME"
        fi
    else
        # Default to 'bot' (gateway) if nothing specified, to match default behavior
        # But if we want ALL logs when nothing is specified, we should leave it empty.
        # However, get-logs.sh calls this for "Main" without args.
        # Let's assume "Main" means gateway/bot.
        CW_CONTAINER="bot"
    fi

    # Construct output filename
    if [ -n "$CW_CONTAINER" ]; then
        # Rename 'bot' to 'gateway' in filename for clarity
        FILENAME_PART="$CW_CONTAINER"
        if [ "$CW_CONTAINER" == "bot" ]; then
            FILENAME_PART="gateway"
        fi
        OUTPUT_FILE="${OUTPUT_DIR}/${NAMESPACE}-${FILENAME_PART}-cloudwatch.txt"
        CONTAINER_ARG="--container $CW_CONTAINER"
    else
...
if [ -n "$SERVICE_NAME" ]; then
    ...
    if [ -z "$POD_NAME" ]; then
        echo -e "${YELLOW}[DEBUG]${NC} Trying label selector: app.kubernetes.io/component=$SERVICE_NAME"
        POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/component="$SERVICE_NAME" -o jsonpath='{.items[*].metadata.name}' 2>/dev/null | awk '{print $1}')
    fi
...
# Function to resolve a friendly filename component (align with CloudWatch naming)
# Examples:
#   service=gateway -> gateway
#   service=operator -> operator
#   service=warm-pool -> runner
#   container=bot -> gateway (matches cloudwatch naming)
#   container=runner -> runner
resolve_filename_part() {
    local container="$1"
...
get_container_logs() {
    ...
    # Align filename with CloudWatch style: namespace-component[-previous].txt
    local friendly
    friendly=$(resolve_filename_part "$container")
    local output_file="${OUTPUT_DIR}/${NAMESPACE}-${friendly}${suffix}.txt"
    ...
    # Add tail option (not compatible with --previous in some cases, so skip for previous)
    if [ -n "$TAIL_LINES" ] && [ "$TAIL_LINES" != "0" ] && [ "$previous" = false ]; then
        kubectl_cmd="$kubectl_cmd --tail=$TAIL_LINES"
    fi
    
    # Add since option (not compatible with --previous)
    if [ -n "$SINCE" ] && [ "$previous" = false ]; then
        kubectl_cmd="$kubectl_cmd --since=$SINCE"
    fi
    
    # Add follow option (not compatible with --previous)
    if [ "$FOLLOW" = true ] && [ "$previous" = false ]; then
        kubectl_cmd="$kubectl_cmd -f"
    fi
    
    local log_type="current"
    if [ "$previous" = true ]; then
        log_type="previous (crashed/terminated)"
    fi
    
    print_info "Getting $log_type logs for container '$container'..."
    print_info "  Command: $kubectl_cmd"
    print_info "  Output: $output_file"

    # Execute the command to a temp file first so we can inspect failures
    local tmpfile
    tmpfile=$(mktemp)
    if eval "$kubectl_cmd" > "$tmpfile" 2>&1; then
        mv "$tmpfile" "$output_file"
        print_success "Logs for container '$container' ($log_type) saved to: $output_file"
        return 0
    else
        local err_msg
        err_msg=$(cat "$tmpfile")
        rm -f "$tmpfile"
        if [ "$previous" = true ] && echo "$err_msg" | grep -qi "previous terminated container \"$container\".*not found"; then
            # Don't emit a useless file when previous logs don't exist
            print_warning "No previous logs available for container '$container' (container has not restarted)"
            return 0
        fi
        if [ "$previous" = true ]; then
            print_warning "No previous logs available for container '$container' (container may not have restarted)"
        else
...
if [ -n "$CONTAINER_NAME" ]; then
    if [ "$FOLLOW" = true ]; then
        ...
        # Get current logs
        get_container_logs "$CONTAINER_NAME" false
        
        # Try to get previous logs
        echo ""
        print_info "Attempting to retrieve previous/crashed container logs..."
        get_container_logs "$CONTAINER_NAME" true
    fi
else
    ...
    # Iterate through all containers - get both current and previous logs
    SUCCESS_COUNT=0
    FAIL_COUNT=0
    PREVIOUS_COUNT=0
    ...
    for container in $CONTAINERS; do
        # Get current logs
        if get_container_logs "$container" false; then
            SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
        else
            FAIL_COUNT=$((FAIL_COUNT + 1))
        fi
        
        # Try to get previous logs (from crashed/terminated container)
        echo ""
        print_info "Attempting to retrieve previous/crashed container logs..."
        if get_container_logs "$container" true; then
    ...
    echo "----------------------------------------"
    print_success "Retrieved current logs from $SUCCESS_COUNT container(s)"
    if [ $PREVIOUS_COUNT -gt 0 ]; then
        print_success "Retrieved previous/crashed logs from $PREVIOUS_COUNT container(s)"
    else
        print_info "No previous/crashed container logs available (containers have not restarted)"
    fi
