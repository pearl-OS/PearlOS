...
# Script to get current logs from the mesh-stg workspace service container
# Usage: ./scripts/get-mesh-stg-logs.sh [options]
...
Get current logs from the mesh-stg workspace service container.
...
OPTIONS:
    ...
    -n, --namespace NAME   Kubernetes namespace (default: mesh-{env})
...
get_container_logs() {
    ...
    local previous_output_file="${OUTPUT_DIR}/${NAMESPACE}-${container}-previous.txt"
    ...
    if eval "$kubectl_cmd" > "$output_file" 2>&1; then
        ...
        # Also try to get previous logs (from crashed/restarted containers)
        local previous_kubectl_cmd="kubectl logs -n $NAMESPACE $POD_NAME -c $container --previous"
        if [ -n "$TAIL_LINES" ] && [ "$TAIL_LINES" != "0" ]; then
            previous_kubectl_cmd="$previous_kubectl_cmd --tail=$TAIL_LINES"
        fi
        
        print_info "  Attempting to get previous logs (if container restarted)..."
        if eval "$previous_kubectl_cmd" > "$previous_output_file" 2>&1; then
            print_success "  Previous logs saved to: $previous_output_file"
        else
            print_info "  No previous logs available (container may not have restarted)"
            rm -f "$previous_output_file" 2>/dev/null
        fi
