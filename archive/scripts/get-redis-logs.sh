...
# Script to get current logs from the redis-stg service container
# Usage: ./scripts/get-redis-stg-logs.sh [options]
...
Get current logs from the redis-stg service container.
...
OPTIONS:
    ...
    -n, --namespace NAME   Kubernetes namespace (default: redis-{env})
...
if [ -n "$SERVICE_NAME" ]; then
    ...
    # For Redis, also try redis-specific labels
    if [ -z "$POD_NAME" ]; then
        echo -e "${YELLOW}[DEBUG]${NC} Trying label selector: app.kubernetes.io/name=redis"
        POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=redis -o jsonpath='{.items[*].metadata.name}' 2>/dev/null | awk '{print $1}')
    fi
    
    if [ -z "$POD_NAME" ]; then
        echo -e "${YELLOW}[DEBUG]${NC} Trying label selector: app.kubernetes.io/instance=$SERVICE_NAME"
        POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/instance="$SERVICE_NAME" -o jsonpath='{.items[*].metadata.name}' 2>/dev/null | awk '{print $1}')
    fi
    
    if [ -z "$POD_NAME" ]; then
        echo -e "${YELLOW}[DEBUG]${NC} Trying label selector: app.kubernetes.io/component=master"
        POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/component=master -o jsonpath='{.items[*].metadata.name}' 2>/dev/null | awk '{print $1}')
    fi
else
